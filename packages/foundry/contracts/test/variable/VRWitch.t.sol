...
abstract contract WitchStateZero is Test, TestConstants {
    ...
    bytes6 internal constant SERIES_ID = bytes6(0);
    ...
    VRWitch internal witch;
    IVRWitch internal iWitch;
    ...
    function setUp() public virtual {
        ...
        witch.grantRole(VRWitch.point.selector, ada);
        witch.grantRole(VRWitch.setLineAndLimit.selector, ada);
        witch.grantRole(VRWitch.setProtected.selector, ada);
        witch.grantRole(VRWitch.setAuctioneerReward.selector, ada);
        vm.stopPrank();
...
contract WitchStateZeroTest is WitchStateZero {
    ...
    function testPointRequiresLadle() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.UnrecognisedParam.selector,
                bytes32("cauldron")
            )
        );
    ...
    function testSetAuctioneerRewardTooHigh() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.AuctioneerRewardTooHigh.selector,
                1e18,
                1.00001e18
            )
        );
...
contract WitchWithMetadataTest is WitchWithMetadata {
    ...
    function testWitchIsTooOld() public {
        ...
        vm.expectRevert(VRWitch.WitchIsDead.selector);
    ...
    function testVaultNotUndercollateralised() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.NotUnderCollateralised.selector,
                VAULT_ID
            )
        );
        witch.auction(VAULT_ID, bot);
    ...
    function testVaultIsProtected() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.VaultAlreadyUnderAuction.selector,
                VAULT_ID,
                owner
            )
        );
    ...
    function testAuctionAVaultWithoutLimitsSet() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.VaultNotLiquidatable.selector,
                ILK_ID,
                BASE_ID
            )
        );
    ...
    function testCancelNonExistentAuction() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.VaultNotUnderAuction.selector,
                VAULT_ID
            )
        );
...
contract WitchWithAuction is WitchWithMetadata {
    ...
    function setUp() public virtual override {
        ...
        vm.prank(bot);
        (auction, vault) = witch.auction(VAULT_ID, bot);
    ...
    function testCollateralLimits() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.CollateralLimitExceeded.selector,
                sum,
                max
            )
        );
    ...
    function testDustLimitProportionUnderDust() public {
        ...
        assertEq(auction2.baseId, BASE_ID);
    ...
    function testCancelUndercollateralisedAuction() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.UnderCollateralised.selector,
                VAULT_ID
            )
        );
        witch.cancel(VAULT_ID);
    ...
    function testClearOwnedAuction() public {
        vm.expectRevert(
            abi.encodeWithSelector(VRWitch.AuctionIsCorrect.selector, VAULT_ID)
        );
    ...
    function testPayBaseNotEnoughBought() public {
        ...
        cauldron.debtFromBase.mock(vault.baseId, maxBaseIn, maxBaseIn);
        cauldron.debtToBase.mock(vault.baseId, maxBaseIn, maxBaseIn);

        vm.expectRevert(
            abi.encodeWithSelector(
                VRWitch.NotEnoughBought.selector,
                minInkOut,
                35.7 ether
            )
        );
    ...
    function testPayBaseLeavesDust() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(VRWitch.LeavesDust.selector, 4999e6, 5000e6)
        );
