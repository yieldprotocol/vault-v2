...
contract VRCauldron is AccessControl(), Constants {
    ...
    event VaultBuilt(bytes12 indexed vaultId, address indexed owner, bytes6 indexed baseId, bytes6 ilkId);
    event VaultTweaked(bytes12 indexed vaultId, bytes6 indexed baseId, bytes6 indexed ilkId);
    ...
    /// @dev Add a new base
    function addBase(bytes6 baseId)
        external
        auth
    {
    ...
    function addIlks(bytes6 baseId, bytes6[] calldata ilkIds)
        external
        auth
    {
        require (
    ...
    function build(address owner, bytes12 vaultId, bytes6 baseId, bytes6 ilkId)
        external
        auth
    ...
    function _tweak(bytes12 vaultId, bytes6 baseId, bytes6 ilkId)
        internal
        returns(DataTypes.VRVault memory vault)
    {
    ...
    /// @dev Change a vault base and/or collateral types.
    /// We can change the base if there is no debt, or assets if there are no assets
    function tweak(bytes12 vaultId, bytes6 baseId, bytes6 ilkId)
        external
        auth
        returns(DataTypes.VRVault memory vault)
    {
    ...
    function vaultData(bytes12 vaultId)
        internal
        view
        returns (DataTypes.VRVault memory vault_, DataTypes.Balances memory balances_)
    {
    ...
    function pour(bytes12 vaultId, int128 ink, int128 base)
        external
        virtual
        auth
    ...
    function slurp(bytes12 vaultId, uint128 ink, uint128 base)
        external
        auth
        returns (DataTypes.Balances memory)
    {
