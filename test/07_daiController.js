const DaiController = artifacts.require('./DaiController');
...
contract('DaiController', async (accounts) =>  {
    ...
    beforeEach(async() => {
        ...
        // Setup daiController
        daiController = await DaiController.new(
            weth.address,       // weth
            dai.address,        // dai
            wethJoin.address,   // wethJoin
            daiJoin.address,    // daiJoin
        ...
        await vat.rely(daiController.address, { from: owner }); //?

        await yDai.grantAccess(daiController.address, { from: owner });
        await lender.grantAccess(daiController.address, { from: owner });
        await saver.grantAccess(daiController.address, { from: owner });
    });
    ...
    it("allows user to post collateral", async() => {
        ...
        await weth.approve(daiController.address, amount, { from: user }); 
        await daiController.post(user, amount, { from: user });
        ...
        let ink = (await vat.urns(ilk, daiController.address)).ink.toString()
        assert.equal(
            ink,   
            amount
        );
    ...
    describe("with posted collateral", () => {
        ...
        it("allows user to withdraw collateral", async() => {
            ...
            let amount = web3.utils.toWei("500");
            await daiController.withdraw(user, amount, { from: user });
        ...
        it("internally allows to borrow dai", async() => {
            ...
            await daiController.borrowDai(user, daiBorrowed, { from: owner });
            ...
            // assert daiController debt = daiBorrowed
        });
        ...
        it("internally allows to save dai in the Pot", async() => {
            ...
            // assert daiController debt = daiBorrowed
            /* assert.equal(
                (await vat.dai(daiController.address)).toString(),   
                daiBorrowed
            ); */ // Not sure if I'm not checking daiController dai debt right
        });

        it("borrows dai if there is none in the Pot", async() => {
            let daiBorrowed = web3.utils.toWei("100");
            await daiController.disburse(user, daiBorrowed, { from: user });
        ...
        describe("with a dai debt towards MakerDAO", () => {
            ...
            it("internally repays Dai debt and no more", async() => {
                ...
                let daiBorrowed = web3.utils.toWei("100");
                await dai.transfer(daiController.address, daiBorrowed, { from: user });
                await daiController.repayDai({ from: owner });
                let daiBalance = (await dai.balanceOf(user)).toString();
                assert.equal(
                    daiBalance,   
                    0
                );
                // assert daiController debt = 0
                assert.equal(
                    (await vat.dai(daiController.address)).toString(),   
                    0
                );
            ...
            it("repays dai if there is a debt, but no more", async() => {
                // Test `normalizedAmount >= normalizedDebt`
                let daiBorrowed = web3.utils.toWei("100");
                await dai.approve(daiController.address, daiBorrowed, { from: user });
                await daiController.repay(user, daiBorrowed, { from: user });
                let daiBalance = (await dai.balanceOf(user)).toString();
                assert.equal(
                    daiBalance,   
                    0
        ...
        describe("without a dai debt towards MakerDAO", () => {
            beforeEach(async() => {
                // Generate dai without generating debt for daiController
            ...
            it("internally transfers all dai into the Pot", async() => {
                ...
                let daiBorrowed = web3.utils.toWei("10");
                await dai.transfer(daiController.address, daiBorrowed, { from: owner });
                await daiController.lockDai({ from: owner });
                assert.equal(
                    (await pot.pie(daiController.address)).toString(),   
                    daiBorrowed
                );
            });
            ...
            describe("with dai in the Pot", () => {
                ...
                it("internally retrieves dai from the Pot", async() => {
                    let daiBorrowed = web3.utils.toWei("10");
                    await daiController.freeDai(daiBorrowed, { from: owner });
                    assert.equal(
                        (await pot.pie(daiController.address)).toString(),   
                        0
                    );
