...
contract('Controller: Multi-Series', async (accounts) => {
  ...
  it('adds series', async () => {
    assert.equal(await controller.containsSeries(maturity1), false, 'Controller should not contain any maturity')

    await controller.addSeries(yDai1.address, { from: owner })

    assert.equal(await controller.containsSeries(maturity1), true, 'Controller should contain ' + (await yDai1.name()))
  })
  ...
  it('adds several series', async () => {
    ...
    assert.equal(await controller.containsSeries(maturity1), true, 'Controller should contain ' + (await yDai1.name()))
    assert.equal(await controller.containsSeries(maturity2), true, 'Controller should contain ' + (await yDai2.name()))
    assert.equal(
  ...
  it("can't add same series twice", async () => {
    await controller.addSeries(yDai1.address, { from: owner })
    await expectRevert(controller.addSeries(yDai1.address, { from: owner }), 'Controller: Series already added')
  })
})
