...
describe('FYToken', () => {
  ...
  describe('after maturity', async () => {
    ...
    it('matures by recording the chi value', async () => {
      ...
      expect(await fyToken.mature()).to.emit(chiOracle, 'Recorded').withArgs(maturity, RAY)
    })
    ...
    describe('once matured', async () => {
      ...
      it('redeems fyToken for underlying according to the chi accrual', async () => {
        ...
        await expect(fyToken.redeem(owner, WAD)).to.emit(fyToken, 'Redeemed').withArgs(owner, owner, WAD, WAD.mul(accrual).div(RAY))
