...
library Math {
    ...
    function min(uint128 x, uint128 y) external pure returns (uint128) {
        return x < y ? x : y;
    }
}
...
library RMath {
    ...
    function rdiv(uint128 x, uint128 y) external pure returns (uint128 z) {
    ...
    function rdivup(uint128 x, uint128 y) external pure returns (uint128 z) {
...
contract Witch {
    ...
    function buy(bytes12 vaultId, uint128 dart, uint128 min)
        public
    {
        ...
        uint128 dink = dart.rdivup(price(balances.ink, balances.art, elapsed));                 // calculate collateral to sell. Using divdrup stops rounding from leaving 1 stray wei in vaults.
    ...
    function price(uint128 ink, uint128 art, uint128 elapsed) public pure returns (uint128) {
        ...
        return RAY.rdiv(term1.rmul(term2 + term3));
    }
}