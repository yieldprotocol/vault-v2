...
contract PoolFactoryMock is IPoolFactory {
  ...
  bytes32 public constant override POOL_BYTECODE_HASH = keccak256(type(PoolMock).creationCode);
  ...
  function createPool(address base, address fyToken) external override returns (address) {
    ...
    PoolMock pool = new PoolMock{salt: keccak256(abi.encodePacked(base, fyToken))}();
    _nextBase = address(0);
